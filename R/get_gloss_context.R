#' @title Searching the occurrence contexts of a gloss in a dataset
#'
#' @description
#' `get_gloss_contexts` is a `meanalyzeR` UI function that returns the set of data or contexts in which a given gloss
...
#'
#' @param gloss A character string representing the gloss to search for (case-sensitive).
...
#'
#' @returns The list of data (if `mode = "full"` is ) or the list of contexts (if `mode = "contexts"` is chosen) in "file" in which the gloss appears. Optionally exports the results to .tex, .wriml, .txt or .rmd. Note that: ".tex"-export is not available in the `contexts` mode.
#'
#' @export
get_gloss_contexts <- function(gloss, file, mode = "full") {
  # Lecture du fichier
  if (!file.exists(file)) {
    stop("File not found.")
  }
  ...
  # Fonction de détection de la glose
  contains_gloss <- function(block, gloss) {
    # Extraire la ligne de gloss
    gl_line <- block[grepl("^\\^gl\\s", block)]
    if (length(gl_line) == 0) return(FALSE)

    # Nettoyer la ligne sans altérer la casse
    gl_text <- gsub("^\\^gl\\s*|\\s*_gl$", "", gl_line)
    gl_text <- trimws(gl_text)

    # Découper en jetons
    tokens <- unlist(strsplit(gl_text, "[[:space:][:punct:]]+"))
  ...
  matched_blocks <- Filter(function(block) contains_gloss(block, gloss), data_blocks)

  if (length(matched_blocks) == 0) {
    cat("No data found for the gloss:", gloss, "\n")
  ...
  if (export_choice == "y") {
    ...
    output_file <- paste0("export/", base_name, "_gloss-", gloss, "_contexts_report", file_ext)
